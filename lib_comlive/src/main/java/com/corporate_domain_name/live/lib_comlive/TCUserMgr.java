...
import com.corporate_domain_name.live.lib_comlive.liveroom.IMLVBLiveRoomListener;
import com.corporate_domain_name.live.lib_comlive.liveroom.MLVBLiveRoom;
...
public class TCUserMgr {
    ...
    private void loginInternal(final String userId, final String pwd, final TCHTTPMgr.Callback callback) {
        try {
            ...
            TCHTTPMgr.getInstance().request("", body, new TCHTTPMgr.Callback() {
                @Override
...
//                            TCELKReportMgr.getInstance().reportELK(TCConstants.ELK_ACTION_LOGIN, userId, -2, msg, null);
    ...
    public void fetchUserInfo(final TCHTTPMgr.Callback callback) {
        JSONObject body = new JSONObject();
        TCHTTPMgr.getInstance().requestWithSign("" + "/get_user_info", body, new TCHTTPMgr.Callback() {
            @Override
            public void onSuccess(JSONObject data) {
                if (data != null) {
    ...
    public void uploadUserInfo(final TCHTTPMgr.Callback callback) {
        try {
            ...
            TCHTTPMgr.getInstance().requestWithSign("" + "/upload_user_info", body, callback);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
